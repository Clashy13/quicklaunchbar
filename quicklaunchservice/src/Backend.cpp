...
Backend::Backend( QObject* parent ) : QObject( parent ) {
    connect( this, &Backend::hideWindow, this, [ this ]() {
        this->_profileManager.currentProfileIndex = -1;
    } );
...
void Backend::toggleProfile( qsizetype profileIndex ) {
    const auto newCurrentScreen = this->newCurrentScreen();
    if ( profileIndex == this->_profileManager.currentProfileIndex ) {
        if ( newCurrentScreen == this->_currentProfileScreen ) {
            emit this->hideWindow();
            this->_profileManager.currentProfileIndex = -1;
        } else {
            this->_currentProfileScreen = newCurrentScreen;
            emit this->showProfile( this->_profileManager.qmlProfileAt( profileIndex ) );
        }
    } else {
        this->_profileManager.currentProfileIndex = profileIndex;
        if ( newCurrentScreen != this->_currentProfileScreen ) {
            this->_currentProfileScreen = newCurrentScreen;
        }
        emit this->showProfile( this->_profileManager.qmlProfileAt( profileIndex ) );
...
QPoint Backend::contentPosition( const qsizetype width, const qsizetype height ) const {
    if ( !this->_profileManager.hasCurrentProfile() || this->_currentProfileScreen == nullptr ) {
        return QPoint( 0, 0 );
    }

    const auto view = this->_profileManager.currentProfileView();
...
qsizetype Backend::availableContentWidth() {
    ...
    const auto margin = this->_profileManager.hasCurrentProfile()
                            ? this->_profileManager.currentProfileView().margin
                            : 0;
